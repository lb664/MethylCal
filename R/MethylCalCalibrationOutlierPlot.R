#' MethylCal calibration outlier plots
#'
...
#' @export
#'
#' @return This function detects the presence of outliers in the
#' Target dataset, defined as the values outside the posterior predictive 
#' interval [l, u] with l = Q1 - 1.5 IQR and u = Q3 + 1.5 IQR with 
#' IQR = Q3 - Q1 and Q3 and Q1 the 75th and 25th percentiles. For
#' a normal posterior distribution [l, u] = [-2.698 std, 2.698 std]
#' with sdt the variance of the posterior predictive distribution. 
#'
#' Three plots are generated with black dots if outliers are detected.
#' In the first plot, the values of the recorded apparent methylation
#' levels at each Actual Methylation Percentage (AMP) are depicted with
#' superimposed MethylCals' calibration curve (Ochoa et al., 2019) 
#' for each CpG (red dashed line). The second plot presents the apparent
#' methylation levels at consecutive CpGs stratified by AMPs with 
#' superimposed the predicted values (red dashed line) as well as the
#' (1-level)\% posterior predictive region (dashed-dotted red lines).
#' Finally, the third plot is the scatterplot of the corrected actual
#' methylation percentage at each AMP for all CpGs within a DRM/CpG 
#' island/gene. If outliers are detected the models will be refitted
#' after removing the outliers as well as the plots above.
#'
...
MethylCalCalibrationOutlierPlot = function(data = data, Target = NULL, prior = "LG", level = 0.95, dir = NULL, printing = TRUE, cex_par = 1.25)
{
    ...
    if (MethylCal_idx == 1)
    {
        ...
        fit_MethylCal_1$summary.fitted.values$sd[fit_MethylCal_1$summary.fitted.values$sd < sd_min] = sd_min
        predict_MethylCal_CI_outlier_low = fit_MethylCal_1$summary.fitted.values$mean - q_outlier * fit_MethylCal_1$summary.fitted.values$sd
        predict_MethylCal_CI_outlier_high = fit_MethylCal_1$summary.fitted.values$mean + q_outlier * fit_MethylCal_1$summary.fitted.values$sd
        predict_MethylCal_CI_outlier_low[predict_MethylCal_CI_outlier_low < 0] = 0
        predict_MethylCal_CI_outlier_low[predict_MethylCal_CI_outlier_low > 100] = 100
        predict_MethylCal_CI_outlier_high[predict_MethylCal_CI_outlier_high < 0 ] = 0
        predict_MethylCal_CI_outlier_high[predict_MethylCal_CI_outlier_high > 100] = 100
    }
        if (MethylCal_idx == 2)
    {
        for (s in unique(data$CpG_group))
        ...
        fit_MethylCal_2$summary.fitted.values$sd[fit_MethylCal_2$summary.fitted.values$sd < sd_min] = sd_min
        predict_MethylCal_CI_outlier_low = fit_MethylCal_2$summary.fitted.values$mean - q_outlier * fit_MethylCal_2$summary.fitted.values$sd
        predict_MethylCal_CI_outlier_high = fit_MethylCal_2$summary.fitted.values$mean + q_outlier * fit_MethylCal_2$summary.fitted.values$sd
        predict_MethylCal_CI_outlier_low[predict_MethylCal_CI_outlier_low < 0] = 0
        predict_MethylCal_CI_outlier_low[predict_MethylCal_CI_outlier_low > 100] = 100
        predict_MethylCal_CI_outlier_high[predict_MethylCal_CI_outlier_high < 0 ] = 0
        predict_MethylCal_CI_outlier_high[predict_MethylCal_CI_outlier_high > 100] = 100
    ...
    if (MethylCal_idx == 3)
    {
        ...
        fit_MethylCal_3$summary.fitted.values$sd[fit_MethylCal_3$summary.fitted.values$sd < sd_min] = sd_min
        predict_MethylCal_CI_outlier_low = fit_MethylCal_3$summary.fitted.values$mean - q_outlier * fit_MethylCal_3$summary.fitted.values$sd
        predict_MethylCal_CI_outlier_high = fit_MethylCal_3$summary.fitted.values$mean + q_outlier * fit_MethylCal_3$summary.fitted.values$sd
        predict_MethylCal_CI_outlier_low[predict_MethylCal_CI_outlier_low < 0] = 0
        predict_MethylCal_CI_outlier_low[predict_MethylCal_CI_outlier_low > 100] = 100
        predict_MethylCal_CI_outlier_high[predict_MethylCal_CI_outlier_high < 0 ] = 0
        predict_MethylCal_CI_outlier_high[predict_MethylCal_CI_outlier_high > 100] = 100
    ...
    if (MethylCal_idx == 4)
    {
        ...
        fit_MethylCal_4$summary.fitted.values$sd[fit_MethylCal_4$summary.fitted.values$sd < sd_min] = sd_min
        predict_MethylCal_CI_outlier_low = fit_MethylCal_4$summary.fitted.values$mean - q_outlier * fit_MethylCal_4$summary.fitted.values$sd
        predict_MethylCal_CI_outlier_high = fit_MethylCal_4$summary.fitted.values$mean + q_outlier * fit_MethylCal_4$summary.fitted.values$sd
        predict_MethylCal_CI_outlier_low[predict_MethylCal_CI_outlier_low < 0] = 0
        predict_MethylCal_CI_outlier_low[predict_MethylCal_CI_outlier_low > 100] = 100
        predict_MethylCal_CI_outlier_high[predict_MethylCal_CI_outlier_high < 0 ] = 0
        predict_MethylCal_CI_outlier_high[predict_MethylCal_CI_outlier_high > 100] = 100
    ...
    if (MethylCal_idx == 5)
    {
        ...
        fit_MethylCal_5$summary.fitted.values$sd[fit_MethylCal_5$summary.fitted.values$sd < sd_min] = sd_min
        predict_MethylCal_CI_outlier_low = fit_MethylCal_5$summary.fitted.values$mean - q_outlier * fit_MethylCal_5$summary.fitted.values$sd
        predict_MethylCal_CI_outlier_high = fit_MethylCal_5$summary.fitted.values$mean + q_outlier * fit_MethylCal_5$summary.fitted.values$sd
        predict_MethylCal_CI_outlier_low[predict_MethylCal_CI_outlier_low < 0] = 0
        predict_MethylCal_CI_outlier_low[predict_MethylCal_CI_outlier_low > 100] = 100
        predict_MethylCal_CI_outlier_high[predict_MethylCal_CI_outlier_high < 0 ] = 0
        predict_MethylCal_CI_outlier_high[predict_MethylCal_CI_outlier_high > 100] = 100
    ...
    outlier_idx = !(predict_MethylCal_CI_outlier_low <= data$y & data$y <= predict_MethylCal_CI_outlier_high)
    outlier = cbind(predict_MethylCal_CI_outlier_low, data$y, fitted_MethylCal[, MethylCal_idx], predict_MethylCal_CI_outlier_high, outlier_idx)
    data$y_outlier = NA
    data$y_outlier_correct_MethylCal = NA
    data$y_outlier[outlier_idx] = data$y[outlier_idx]
    data$y_outlier_correct_MethylCal[outlier_idx] = data$y_correct_MethylCal[outlier_idx]
    ...
    e = lattice::xyplot(y_outlier ~ x, groups = factor(CpG_group), type = "p", pch = 21, lwd = 1.5, cex = 1.05, fill = "black", col = "black", strip = lattice::strip.custom(bg = "white"), data = data)
    p = a + latticeExtra::as.layer(c) + latticeExtra::as.layer(d) + latticeExtra::as.layer(e)
    if (Sys_info[[1]] == "Windows")
    {
        grDevices::dev.new()
        print(p)
    } else {
        name_fig = paste(paste(dir, "/", "MethylCal[CalibrationOulierPlot_", levels(droplevels(data$Target[1])), "_Fig1", sep = ""), "pdf", sep = ".")
    ...
    d = lattice::xyplot(y_outlier ~ CpG_group | factor(AMP_group_label, levels = AMP_level), type = "p", pch = 21, lwd = 1.5, cex = 0.85, fill = "black", col = "black", strip = lattice::strip.custom(bg = "white"), data = data)
    e = lattice::xyplot(fitted_MethylCal[, MethylCal_idx] ~ CpG_group | factor(AMP_group_label, levels = AMP_level), type = "l", lty = 2, lwd = 2, col = "red", strip = lattice::strip.custom(bg = "white"), data = data)
    f = lattice::xyplot(predict_MethylCal_CI_low ~ CpG_group | factor(AMP_group_label, levels = AMP_level), type = "l", lty = 6, lwd = 2, col = "red", strip = lattice::strip.custom(bg = "white"), data = data)
    g = lattice::xyplot(predict_MethylCal_CI_high ~ CpG_group | factor(AMP_group_label, levels = AMP_level), type = "l", lty = 6, lwd = 2, col = "red", strip = lattice::strip.custom(bg = "white"), data = data)
    p = a + latticeExtra::as.layer(b) + latticeExtra::as.layer(c) + latticeExtra::as.layer(d) + latticeExtra::as.layer(e) + latticeExtra::as.layer(f) + latticeExtra::as.layer(g)
    if (Sys_info[[1]] == "Windows")
    {
        grDevices::dev.new()
        print(p)
    } else {
        name_fig = paste(paste(dir, "/", "MethylCal[CalibrationOulierPlot_", levels(droplevels(data$Target[1])), "_Fig2", sep = ""), "pdf", sep = ".")
        grDevices::pdf(file = name_fig)
        print(p)
        grDevices::dev.off()
    }
    ...
    c = lattice::xyplot(y_outlier_correct_MethylCal ~ x, groups = factor(CpG_group), type = "p", pch = 21, lwd = 1.5, cex = 1.05, fill = "black", col = "black", strip = lattice::strip.custom(bg = "white"), data = data)
    ...
    if (Sys_info[[1]] == "Windows")
    {
        ...
        name_fig = paste(paste(dir, "/", "MethylCal[CalibrationOulierPlot_", levels(droplevels(data$Target[1])), "_Fig3", sep = ""), "pdf", sep = ".")
        grDevices::pdf(file = name_fig)
        print(p)
        grDevices::dev.off()
    }

    data$y[outlier_idx] = NA
    ...
    if (any(is.na(data$y)))
    {
        ...
        if (printing == TRUE)
        {
            print("Model 1 after removing outliers...")
        }
        fit_MethylCal_2 = inla(formula_MethylCal_2, family = "gaussian", control.compute = list(dic = TRUE, waic = TRUE), control.predictor = list(compute = TRUE), data = data)
        if (printing == TRUE)
        {
            print("Model 1 after removing outliers estimated")
            print("Model 2 after removing outliers ...")
        }
        fit_MethylCal_3 = inla(formula_MethylCal_3, family = "gaussian", control.compute = list(dic = TRUE, waic = TRUE), control.predictor = list(compute = TRUE), data = data)
        if (printing == TRUE)
        {
            print("Model 2 after removing outliers estimated")
            print("Model 3 after removing outliers ...")
        }
        fit_MethylCal_4 = inla(formula_MethylCal_4, family = "gaussian", control.compute = list(dic = TRUE, waic = TRUE), control.predictor = list(compute = TRUE), data = data)
        if (printing == TRUE)
        {
            print("Model 3 after removing outliers estimated")
            print("Model 4 after removing outliers ...")
        }
        fit_MethylCal_5 = inla(formula_MethylCal_5, family = "gaussian", control.compute = list(dic = TRUE, waic = TRUE), control.predictor = list(compute = TRUE), data = data)
        if (printing == TRUE)
        {
            print("Model 4 after removing outliers estimated")
            cat(rep("\n", 1))
        }
        ...
        if (printing == TRUE)
        {
            print("Estimated models after removing outliers")
            print(c("", "Model 1", "Model 2", "Model 3", "Model 4"))
            print(c("mlik", round(mlik_MethylCal[-DIC_remove], 2)))
            print(c("DIC", round(DIC_MethylCal[-DIC_remove], 2)))
            print(c("RSS", round(RSS_MethylCal[-DIC_remove], 2)))
        ...
        if (printing != TRUE)
        {
            print("Correcting apparent methylation levels after removing outliers...")
        }
        ...
        for (s in 1 : n_CpG)
        {
            ...
            if (!all(is.na(data_CpG$y)))
            {
                ...
                for (l in 1 : n_AMP)
                {
                    CpG_AMP_idx = which(data$CpG_group == s & data$AMP_group == l)
                    
                    if (!is.na(data$y[CpG_AMP_idx]))
                    {
                        y_correct = f_MethylCal(data = data, CpG_group = s, AMP_group = l, MethylCal_idx = MethylCal_idx, prior = prior)
                        y_correct[y_correct < 0] = 0
                        y_correct[y_correct > 100] = 100
                        ...
                        if (printing == TRUE)
                        {
                            print(paste(paste("Correcting apparent methylation levels after removing outliers", round(counter / (n_AMP * n_CpG) * 100, 1), sep = " "), "%", sep = ""))
                        }
                    }
        ...
        print("Correcting apparent methylation levels after removing outliers DONE")
        cat(rep("\n", 1))
        ...
        if (Sys_info[[1]] == "Windows")
        {
            ...
            name_fig = paste(paste(dir, "/", "MethylCal[CalibrationOulierPlot_", levels(droplevels(data$Target[1])), "_Fig4", sep = ""), "pdf", sep = ".")
            grDevices::pdf(file = name_fig)
            print(p)
            grDevices::dev.off()
        }
        ...
        if (Sys_info[[1]] == "Windows")
        {
            ...
            name_fig = paste(paste(dir, "/", "MethylCal[CalibrationOulierPlot_", levels(droplevels(data$Target[1])), "_Fig5", sep = ""), "pdf", sep = ".")
            grDevices::pdf(file = name_fig)
            print(p)
            grDevices::dev.off()
        }
        ...
        if (Sys_info[[1]] == "Windows")
        {
            ...
            name_fig = paste(paste(dir, "/", "MethylCal[CalibrationOulierPlot_", levels(droplevels(data$Target[1])), "_Fig6", sep = ""), "pdf", sep = ".")
            grDevices::pdf(file = name_fig)
            print(p)
            grDevices::dev.off()
        }
